...
int solve(vector<int> &nums, int n) {
    ...
    if (n >= nums.size())return 1000000;
    if (dp[n] != -1)return dp[n];
    else {
        int ans = mod, length = nums[n];
        for (int i = 1; i + n < nums.size() and i <= length; i++) {
            ans = min(1 + solve(nums, i + n), ans);
        }
        return dp[n] = ans;
    }
}
...
int main() {
    ...
    int n;
//    cin >> n;
    vector<int> nums = {5, 6, 4, 4, 6, 9, 4, 4, 7, 4, 4, 8, 2, 6, 8, 1, 5, 9, 6, 5, 2, 7, 9, 7, 9, 6, 9, 4, 1, 6, 8, 8,
                        4, 4, 2, 0, 3, 8, 5};
