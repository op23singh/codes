...
int waysToMakeFair(vector<int> &nums) {
    int sumEven = nums[0], sumOdd = 0, cont = 0;
    vector<int> sum_even(nums.size()), sum_odd(nums.size());
    sum_even[0] = nums[0];
    ...
    if (sum_even[nums.size() - 1] - nums[0] == sum_odd[nums.size() - 1])cont++;
    for (int i = nums.size() - 1; i > 0; i--) {
        cont += ((sum_even[i - 1] + sum_odd[nums.size() - 1] - sum_odd[i])
                 ==
                 (sum_odd[i - 1] + sum_even[nums.size() - 1] - sum_even[i]));
    }
    return cont;
}
