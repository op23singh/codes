...
int solve(int n, int f) {
    if (n == 1 or f == 0 or f == 1)return f;
    if (dp[n][f] != -1)return dp[n][f];
    else {
        int ans = 0, tempans = INT_MAX;
        int start=1,end=f;
        while(start<=end) {
            int mid=start+(end-start)/2;
            int left=solve(n - 1, mid - 1);
            int right=solve(n, f - mid);
            ans=1+max(left,right);
            if(left<right){
                start=mid+1;//since we need the worst case
            }
            else end=mid-1;
            tempans = min(tempans, ans);
        }
        return dp[n][f] = tempans;
    }
}
