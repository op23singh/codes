...
int solve(int n, int number, int copied) {
    ...
    if (n < 0)return -10000;
    if (number == 0) {
        return dp[n] = 1 + solve(n - 1, number + 1, copied);
    } else {
        return dp[n] = max({number + solve(n - 3, number * 2, number), 1 + solve(n - 1, number + 1, copied),
                            (copied > 0) ? (copied + solve(n - 1, number + copied, copied)) : INT_MIN});
    }
}
