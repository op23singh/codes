...
int N;
string solve(int n,string made){
    if(n==N){
        return made;
    }
    else{
        string temp;
        int j=0,ne=made.size();
        while(j<ne){
            int count=1;
            while(j+1<ne and made[j]==made[j+1]){
                count++;
                j++;
                continue;
            }
            if(j+1>=ne || j+1<ne and made[j+1]!=made[j]){
                temp.append(to_string(count)+made[j]);
            }
            j++;
        }
