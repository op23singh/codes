...
int solve(vector<int> &nums, int check, int operation) {
        ...
        for (int i = 0; i < nums.size(); i++) {
            ...
            if ((temp1 & check) == 0) {
                for (int j = 0; j < nums.size(); j++) {
                    int temp2 = 1 << j;
                    if (i != j and (temp2 & check) == 0) {
                        ans1 = ((operation + 1) * (__gcd(nums[i], nums[j]))) +
                               solve(nums, check | temp1 | temp2, operation + 1);
                        ans2 = max(ans1, ans2);
                    }
                }
            }
        }
        return mp[check] = ans2;
    }
}
